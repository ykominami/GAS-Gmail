...
class HeaderTable{
   ...
   */
  constructor(spreadsheet, sheetName, config, tableDef, yklibbConfig, ultimate = false){
    ...
    if( yklibbConfig !== null){
      ...
      this.yklibbConfig = new YKLibb.Config(this.sourceHeader.length, this.sourceHeader, YKLibb.Config.COMPLETE(), ultimate)
    }
    ...
    if (spreadsheet && config && yklibbConfig) {
      ...
      const [worksheet0, header0, totalValues0, headerRange0, dataRowsRange0, totalRange0] = this.setup(spreadsheet, sheetName, config, tableDef, yklibbConfig)
      this.worksheet = worksheet0
      this.header = header0
      this.totalValues = totalValues0
      this.headerRange = headerRange0
      this.dataRowsRange = dataRowsRange0
      if( dataRowsRange0 !== null ){
        this.dataRowsValues = dataRowsRange0.getValues()
   ...
   */
  setup(spreadsheet, sheetName, config, tableDef, yklibbConfig, ultimate=false){
    ...
    const [header0, totalValues0, headerRange0, dataRowsRange0, totalRange0] = this.getRangeForHeaderAndData(worksheet, yklibbConfig, ultimate)
    return [worksheet, header0, totalValues0, headerRange0, dataRowsRange0, totalRange0]
  }
  ...
  registerDataArray0(dataArray, op){
    const ultimate = true
    const [header, totalValues, headerRange, dataRowsRange, totalRange] = this.getRangeForHeaderAndData(this.worksheet, this.yklibbConfig, ultimate)

    if( dataRowsRange !== null ){
      this.dataRowsValues = dataRowsRange.getValues()
      const length = this.dataRowsValues.length
      if( length === 1 ){
        // 配列の要素がすべて空白文字列である場合、何も持っていない配列として扱う
        const array = this.dataRowsValues[0]
        if( array.every( item => item.trim().length === 0 ) ){
          this.dataRowsValues = [[]]
        }
      }
    }
    ...
    this.dataRowsRange = this.totalRange.offset(1, 0, this.totalRange.getHeight() - 1)
    const dataRowsRangeShape = YKLiba.Range.getRangeShape(dataRowsRange)
    const x = 1
  }
  ...
  getCol1(worksheet, yklibbConfig){
    ...
    YKLiblog.Log.debug(`############# HeaderTable getCol1 values=${ JSON.stringify(valuesCol1)} `)
    if( this.dataRowsRangeCol1 === null ){
      return []
    }

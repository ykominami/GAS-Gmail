class Dataregister {
  static registerData(within, name, op, limit, lastDate){
    if( (typeof(within.array) === "undefined") || within.array.length <= 0 ){
      // Logger.log(`gmailregster|registerDate| 0 return`)
  ...
  static getDataSheetRange(sheetname){
    ...
    if( !height ){
      if( !width ){
        ...
        // Logger.log(`getDataSheetRange 0 2`)
        range = dataRange.offset(row, col, 1, width);
      }
    }
    else{
      if( !width ){
        // Logger.log(`getDataSheetRange 0 3`)
        range = dataRange.offset(row, col, height, 1);

      }
      else{
        // Logger.log(`getDataSheetRange 0 4`)
      }
      // else{
  ...
  static registerDataArray(dataArray, sheetname, op){
    ...
    Logger.log(`rangeShape=${JSON.stringify(rangeShape)}` )
    ...
    // Logger.log(`register_data height2=${height2} width2=${width2}`)
    ...
    if(height2 > 0 && width2 > 0){
      ...
      try{
        ...
        // 'e' は発生したエラーオブジェクトです。
        Logger.log("エラーが発生しました: " + e.message); // エラーメッセージを出力
        // または、エラーの詳細をログに出力
